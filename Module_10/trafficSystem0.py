...
class TrafficSystem:
    ...
    def __init__(self):
        ...
        self.signal = tc.Light(10, 8)  # Create the traffic light
    ...
    def snapshot(self):
        ...
        signal_str = self.signal.__str__()
        ...
        print(f"{first_lane_str} {signal_str} {second_lane_str}   {queue_str}")

    def step(self):
        """Take one time step for all components."""
        first_lane_vehicle = self.first_lane.remove_first()  # return first position
        self.first_lane.step()

        if self.signal.is_green() and self.second_lane.get_first():
        ...
        if self.queue and self.second_lane.last_free():
